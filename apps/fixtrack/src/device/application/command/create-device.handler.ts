...
{
  constructor(
    @InjectAggregateRepository(Device)
    private readonly users: AggregateRepository<Device, DeviceId>,
  ...
  async execute(command: CreateDeviceCommand): Promise<Device> {
    ...
    if ((await this.deviceFinder.findById(deviceId)) != null)
      throw DeviceAlreadyExistsError.withId(deviceId);
    if ((await this.deviceFinder.findByModel(deviceModel)) != null)
      throw DeviceAlreadyExistsError.withModel(deviceModel);

    const device: Device = Device.add(
      deviceId,
      deviceModel,
      deviceType,
      deviceBrand
