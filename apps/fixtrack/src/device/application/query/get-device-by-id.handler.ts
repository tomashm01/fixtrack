...
export class GetDeviceByIdHandler implements IQueryHandler<GetDeviceByIdQuery>{
  constructor(@Inject(DEVICE_FINDER) private readonly deviceFinder:DeviceFinder) {}

  async execute(query:GetDeviceByIdQuery): Promise<DeviceDTO|null> {

    const deviceDto:DeviceDTO = await this.deviceFinder.findById(DeviceId.with(query.id));
